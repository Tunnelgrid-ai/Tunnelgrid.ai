...
export async function storeTopics(request: StoreTopicsRequest): Promise<TopicsServiceResponse<Topic[]>> {
  try {
    ...
    console.log(`💾 Storing ${topics.length} topics for audit:`, auditId);
    
    // STEP 1: Handle existing topics if replacement requested
    if (replaceExisting) {
      console.log('🗑️ Removing existing topics for audit:', auditId);
    ...
    const topicsToInsert = topics.map(topic => appTopicToDatabaseTopic(topic, auditId));
        ...
        // STEP 3: Insert topics in database    // Handle RLS by ensuring user context (development workaround)    const { data: { user } } = await supabase.auth.getUser();    if (!user) {      console.log('🔧 No authenticated user found - topics may fail due to RLS policy');      console.log('💡 Consider signing in or disabling RLS on topics table for development');    }
    ...
    console.log(`✅ Successfully stored ${storedTopics.length} topics for audit:`, auditId);
    ...
    console.error('❌ Unexpected error storing topics:', error);
    return {
      success: false,
...
export async function getTopicsForAudit(auditId: string): Promise<TopicsServiceResponse<TopicWithMetadata[]>> {
  try {
    ...
    console.log('📥 Retrieving topics for audit:', auditId);
    ...
    if (!topics || topics.length === 0) {
      console.log('ℹ️ No topics found for audit:', auditId);
      return {
        success: true,
        data: [],
        count: 0
    ...
    console.log(`✅ Retrieved ${appTopics.length} topics for audit:`, auditId);
    ...
    console.error('❌ Unexpected error retrieving topics:', error);
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error occurred'
    };
...
export async function checkTopicsExist(auditId: string): Promise<TopicsServiceResponse<{ exists: boolean; count: number }>> {
  try {
    ...
    console.log('🔍 Checking topics existence for audit:', auditId);
    ...
    if (error) {
      console.error('❌ Error checking topics existence:', error);
      return {
        success: false,
    ...
    console.log(`✅ Topics check complete - Exists: ${exists}, Count: ${topicsCount}`);
    ...
    console.error('❌ Unexpected error checking topics existence:', error);
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error occurred'
    };
