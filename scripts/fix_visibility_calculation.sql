-- Fix the visibility calculation to show percentage of responses containing target brand
-- Instead of total brand mentions / total responses (which can exceed 100%)
...
-- Now drop and recreate the function with fixed calculation
...
DECLARE
    ...
    v_responses_with_target_brand INTEGER;  -- NEW: Count responses that contain target brand
...
BEGIN
    ...
    -- FIXED: Calculate responses that contain target brand (not total mentions)
    SELECT COUNT(DISTINCT r.response_id)
    INTO v_responses_with_target_brand
    FROM audit a
    LEFT JOIN queries q ON a.audit_id = q.audit_id
    LEFT JOIN responses r ON q.query_id = r.query_id
    LEFT JOIN brand_extractions be ON r.response_id = be.response_id
    WHERE a.audit_id = p_audit_id AND be.is_target_brand = TRUE;
    
    -- FIXED: Calculate overall visibility percentage (responses with brand / total responses)
    v_overall_visibility := CASE 
        WHEN v_total_responses > 0 THEN 
            ROUND((v_responses_with_target_brand::DECIMAL / v_total_responses * 100), 2)
        ELSE 0 
    END;
    ...
    -- Detailed persona visibility with individual personas
    SELECT jsonb_build_object(
        ...
        'personas', COALESCE((
            SELECT jsonb_agg(
                jsonb_build_object(
                    'name', p.persona_type,
                    'visibility', CASE 
                        WHEN persona_stats.total_responses > 0 THEN 
                            ROUND((persona_stats.brand_mentions::DECIMAL / persona_stats.total_responses * 100), 2)
                        ELSE 0 
                    END,
                    'totalQueries', persona_stats.total_queries,
                    'totalResponses', persona_stats.total_responses,
                    'brandMentions', persona_stats.brand_mentions
                )
            )
            FROM personas p
            LEFT JOIN (
    ...
    -- Detailed topic visibility with individual topics
    SELECT jsonb_build_object(
        ...
        'topics', COALESCE((
            SELECT jsonb_agg(
                jsonb_build_object(
                    'name', t.topic_name,
                    'visibility', CASE 
                        WHEN topic_stats.total_responses > 0 THEN 
                            ROUND((topic_stats.brand_mentions::DECIMAL / topic_stats.total_responses * 100), 2)
                        ELSE 0 
                    END,
                    'totalQueries', topic_stats.total_queries,
                    'totalResponses', topic_stats.total_responses,
                    'brandMentions', topic_stats.brand_mentions
                )
            )
            FROM topics t
            LEFT JOIN (
